...
import { webResourceAccess } from './logic/web.resource';
...
async function bootstrap() {
  ...
  await app.listen(process.env.PORT, '0.0.0.0', async () => {
    logger.info('应用启动完成', process.env.NODE_ENV, process.env.ROOT_DIR);
  });
}
...
async function startExpressServer() {
  ...
  exporessInstance = expressApp.listen(
    ...
    '0.0.0.0',
    () => {
      logger.info('系统初始化模式', process.env.NODE_ENV, process.env.ROOT_DIR);
    },
  );
}
