...
import { webIsStart } from 'src/logic/web.resource';
...
export class AdminController {
  ...
  async syncConfigToWeb() {
    ...
    if (isWebRunning) {
      ...
      // 调用子进程到 web 目录下执行 pnpm build
      const webDir = join(process.env.ROOT_DIR + '/../web');
      try {
        const stdout = execSync(`cd ${webDir} && pnpm build`);
        logger.info(`pnpm build 输出: ${stdout.toString()}`);
      } catch (error) {
        if (error instanceof Error) {
          logger.error(`执行 pnpm build 时出错: ${error.message}`);
        }
        return ResultUtils.fail('执行 pnpm build 时出错');
      }
